...
| Explore model directions without selecting/checking a submission | Discovery live run | Yes | Yes | Yes | `setup-live --run-mode discovery`, then `run` |
...
- submission-mode convergence patience of three consecutive non-improving trusted full-fidelity iterations; discovery mode records but does not stop on this pressure;
...
- Use `finalize` when the operator explicitly chooses to submit a stopped discovery run, or when submission-mode automatic finalization did not complete. It is idempotent after successful finalization.

...
The production command is intentionally quiet during long external calls. Keep it attached and allow it to return. The default deployment seals seven distinct directions from one planner snapshot, runs their Trae worktrees and candidate executions concurrently, evaluates each with independent evidence and serialized protected-query indices, then sends all independently accepted improvements through one newly gated synthesis candidate before the next round. It persists every handoff and automatically finalizes after a legal stop.

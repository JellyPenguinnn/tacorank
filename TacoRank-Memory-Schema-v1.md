...
TacoRank has exactly three canonical memory authorities:
...
- `kind` is one of `diff`, `trajectory`, `context`, `log`, `checkpoint`, `predictions`, `metrics`, `delta_vector`, `verification_receipt`, `submission`, `report`, `other`.
...
The contract also declares every permitted diagnostic metric, the trusted-best comparison rule, and the experiment-family classification used by lesson staleness. For TacoRank, the classification is derived from the existing family enum:

- `frame`: `objective`, `sampling`;
- `content`: `temporal_history`, `features`, `multitask`, `duration_bias`;
- `capacity`: `model`;
- `composition`: `ensemble`;
- `policy`: `evaluation`;
- `other`: `other`.

The trusted-best comparison rule must keep convergence epsilon separate from the acceptance threshold. The KuaiRand contract uses `eta = max(2 * seed_stderr, 0.0016)` and supersedes the trusted best only when `seed_mean > trusted_best + eta`.
...
```json
{
  ...
  "excluded_categories": ["hidden_final", "invalid", "inconclusive", "secrets"],
...
```json
{
  ...
  "prediction_artifact_id": "art_8f31a9c2",
  "ordered_row_identity_sha256": "<sha256 of ordered row_id,user_id,video_id>",
  "ordered_prediction_sha256": "<sha256 of ordered row identity and exact score>",
  "accepted": true,
  "checks": {
    "header": "pass",
    "row_count": "pass",
...
`population` is `internal_proxy`, `public_validation`, `unbiased_audit`, or `hidden_final`.
...
For `confirmed` or `unstable` stability, `trust` also records `eta_applied`,
`seed_mean`, `seed_stderr`, and `seed_count`. `seed_evidence_event_ids` contains
the verified prior evaluation events; `seed_count` equals that list's length
plus the current evaluation.
...
Unbiased-audit isolation rules:

- `public_query_index` is null.
- It never affects convergence or trusted best.
- It never becomes a parent and never causes `experiment.decided`.
- It is consumed only as trust and integrity evidence for another population.
...
Proxy, smoke, unbiased-audit, invalid, suspicious, and hidden-final results never affect convergence.
...
- orthogonality, defined as `1 - max(delta_correlation_with_accepted_nodes)` and clamped to `[0, 1]`.
...
15. Proxy, smoke, and unbiased-audit results cannot update trusted best or convergence.
...
- **Person 1:** `ExperimentSpec`, method-card schema, evidence eligibility, duplicate key, parent recommendation.
- **Person 2:** envelope validation, append/fold, state transitions, context creation, convergence, idempotency, projections, and parent-eligibility enforcement.

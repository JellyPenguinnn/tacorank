...
Execution, Gate A, Gate B, and adapter failures enter the bounded recovery route. A verified evaluation no-op (`NO_PREDICTION_CHANGE`) is different: it is recorded as a terminal pruned/null experiment and returned to the research tree planner, because an unchanged prediction can be a legitimate null result and is not by itself evidence of a wiring failure. The no-op node cannot become a parent or checkpoint. The tree planner ranks two legal next directions when available: one same-mechanism reimplementation from the last trusted parent, or an independent mechanism. A second no-op for that parent/family/method retires the reimplementation option. Recovery may retry the same commit once, apply an approved runtime adjustment, ask Trae for at most two repair patches, roll back, or abandon explicit adapter failures. Every replacement patch needs a new Gate A receipt. Repeated fingerprints stop repair cycling, and deliberate credential, hidden-label, target-label, or network boundary violations are recorded and terminate the run.

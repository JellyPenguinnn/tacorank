...
Execution, output, and evaluation failures are returned to the bounded recovery policy. A Trae repair produces a new commit and invalidates the prior receipt; the replacement cannot execute until Gate A accepts it. Resource ceilings are rechecked after coding, every execution/evaluation, and each repair. Final commands remain sealed to the exact validation-best commit and cannot feed test output back into planning.

...
参考的通宵运行 `run_20260831T205244Z_37968` 在无人值守的情况下完整执行了生产工作流：

| 项目 | 已观察结果 |
| --- | ---: |
| 规划器迭代数 | 24（预算 50） |
| 受保护 FM 基线 | primary 为 0.6014687564 |
| 已接受链 | `exp_001` 0.60351 → `exp_006` 0.60372 |
| 最佳 full-fidelity 候选 | `exp_021`，primary 为 0.603852（较 FM +0.002384） |
| 收敛（官方 ε=0.002 / N=3 规则） | 自第 4 次迭代起持续满足 |
| 停止原因 | `no_legal_proposal`（去重后的候选前沿已耗尽） |
| Provider token | 25.1 M |
| GPU 小时 / 人工干预 | 0 / 0 |

21 个探索方向中有 19 个在低成本 proxy gate 被拒绝——这正是 fail-closed 可信约定按设计运作的证据。运行在不再存在合法且不重复的假设时停止，这比平台期规则更严格。提交制品是该运行验证集最优的候选（`exp_021`，primary 0.603852）：其测试预测直接来自运行记录的制品目录，并通过官方 `submit.py --check` 验证。控制器自身的 fail-closed 选择约定选中了 `exp_001`（最佳的已接受实验）；其最后打包步骤遇到挂载配置 bug（已在本仓库修复）。作为仅供离线分析（未提交）的结果：运行产出成员的按用户 z-score ensemble 在验证集约达 0.6047，支持下文的合规上限论证。
...
在本次比赛配置下，整个方法组合受一条硬性合规规则约束：**所有训练信号与特征必须仅来自训练划分。** 触及评分时段行的滚动或评测窗口反馈特征被禁止，且该禁令在三个独立位置强制执行——规划器提示词、方法卡前置条件和静态代码 gate。（组合中保留了一张滚动反馈方法卡，仅适用于 serving 约定明确允许评分时使用更早行反馈的场景；在本比赛约定下不可选择，参考运行也从未执行它。）

参考运行实际探索并沉淀的方向：

1. **仅用训练划分的因果历史。** 用户、item 和交互聚合特征完全由训练窗口行计算，并通过训练期末尾状态映射到评分人群。
2. **多样化紧凑排序器。** 小型 LambdaRank 与 `rank_xendcg` 成员在 CPU 预算内以不同归纳偏置优化用户内排序；基于 FM 基线的按用户 z-score 残差融合产生了第一个被接受的增益（`exp_001`）。
3. **容量受控的精炼。** 紧凑小容量排序器（`exp_006`）与其精炼变体（`exp_021`）延续了已接受链。
4. **成员 ensemble。** 已接受成员的等权按用户 z-score 组合；ensemble 候选在显式内存纪律策略（成员顺序训练）下运行。
5. **拒绝薄弱或不安全证据。** 泄漏、顺序不明确、增益过度集中，或相对父实验没有超过 `epsilon` 的可信 full-fidelity 改进，都会证伪该方法。

已审核的方法组合位于 [`research/methods/`](research/methods/)；测量该基准合规上限（验证集约 0.6035–0.6047）的离线研究位于 [`research/offline_ceiling_study/`](research/offline_ceiling_study/)。
...
参考运行 ID 为 `run_20260831T205244Z_37968`。运行证据被有意排除在 Git 之外，因此新的 clone **不包含**该账本。若要验证这次历史运行，请先取得完整、未经修改的证据目录（随团队提交包一同分发），并将其保留在：
...
状态必须显示 `status=stopped`、停止原因 `no_legal_proposal`、最佳实验 `exp_021`（primary 0.603852）以及基线 0.6014687564。提交的 CSV 是该账本 `artifacts/` 目录中 `exp_021` 的测试预测制品，并通过官方 `submit.py --check` 验证。控制器的 fail-closed 选择约定选中了 `exp_001`（最佳已接受实验）；其最后打包步骤因挂载配置 bug 失败（已修复），因此该账本中的 `final_experiment_id` 保持未设置。若要从已验证账本重新生成便于阅读的视图，请运行：
...
| 已接受链 | `exp_001` 0.60351 → `exp_006` 0.60372 |
| 最佳 full-fidelity 候选 | `exp_021`，primary 0.603852 |
| 规划器迭代数 | 24（预算 50） |
| 收敛（ε=0.002 / N=3） | 自第 4 次迭代起满足 |
...
已接受链单调上升，因为每次接受都要求候选相对其父实验取得经 seed 确认、且超出按基线自身 seed 方差校准的噪声带的改进。21 个探索方向中有 19 个在低成本 proxy gate 被拒绝；运行在规划器去重后的候选前沿耗尽时停止。
...
- 为已接受的残差融合链及其各排序器成员增加自动 ablation 报告。

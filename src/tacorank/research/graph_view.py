...
class ExperimentNodeView:
    ...
    def is_trusted(self) -> bool:
        verdict = str(self.trust_verdict or "").lower()
        integrity = str(self.integrity or "").lower()
    ...
    def is_parent_eligible(self) -> bool:
        ...
        if explicit is False:
            return False
        status = str(get_value(self.summary, "status", "")).lower()
        if status in {"retracted", "pruned", "invalid", "suspicious", "unstable"}:
            return False
        if str(self.decision or "").lower() in {"reject", "prune", "invalid"}:
            return False
        ...
        if self.is_root:
            ...
            return explicit is True or str(self.decision or "").lower() in {
                "accept",
                "accepted",
                "promote",
                "verified",
        ...
        if explicit is True:
            return True
        return str(self.highest_completed_fidelity or "").lower() == "full"
...
class GraphView:
    ...
    def from_context(cls, context: Any) -> "GraphView":
        ...
        for summary in summaries:
            ...
            trust_verdict = get_value(trust, "verdict", trust)
            ...
            nodes.append(
                ExperimentNodeView(
                    ...
                    integrity=integrity,
                    decision=get_value(summary, "decision", None),
                    highest_completed_fidelity=get_value(summary, "highest_completed_fidelity", None)
                    or get_value(summary, "highest_fidelity_completed", None),
                    primary_score=score,
                    child_count=int(get_value(summary, "child_count", 0) or 0),
                    actual_cost=get_value(summary, "actual_cost", None),
                    summary=summary,
                )
            )
        return cls(nodes)

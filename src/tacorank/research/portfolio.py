...
def default_portfolio() -> ExperimentPortfolio:
    ...
    return ExperimentPortfolio(
        cards=[
            ...
            MethodCard(
                method_id="model_lgbm_causal_history",
                family="model",
                summary="Small LambdaRank GBDT over strictly-past history, LOO target encodings, and static item statistics; replaces the served score.",
                tags=("lightgbm", "lambdarank", "causal_history", "target_encoding", "replacement_capable", "model"),
                cost_tier="medium",
                mechanism="Causal per-impression history plus LOO encodings plus item statistics carry signal the FM cannot see; sibling study measured 0.6056-0.6122 valid vs the 0.6016 parent.",
                ...
                expected_effect="Clearly positive full-fidelity delta from new signal classes.",
                falsifier="No trusted improvement, or one feature absorbing all gain with a collapsed score (leak signature).",
                prohibition_conditions=("evaluator_or_split_change_required",),
            ),
            MethodCard(
                ...
                cost_tier="medium",
                mechanism="Ordered target statistics handle high-cardinality ids without leakage; sibling measurement 0.6120 test standalone.",
                prerequisites=("baseline_parity", "objective_data_frame_verified"),
                allowed_data=(
                    "train_interactions",
                    "user_id",
                ...
                expected_effect="A diverse individually strong ranking member.",
                falsifier="No trusted improvement standalone and no gain when blended.",
                prohibition_conditions=("evaluator_or_split_change_required",),
            ),
            MethodCard(
                method_id="ensemble_zblend_diverse",
                family="ensemble",
                summary="Per-user z-score equal-weight blend of accepted mechanism-diverse members.",
                tags=("ensemble", "zscore", "diversity", "blend"),
                cost_tier="low",
                mechanism="Z-scoring inside each user combines only within-user ordering information; sibling study reached 0.6172 valid vs 0.6120 best member.",
                prerequisites=("baseline_parity", "two_confirmed_clean_members"),
                allowed_data=(
                    "train_interactions",
                    "user_id",
                    "video_id",
                ...
                expected_effect="About +0.005 over the best member when members are diverse.",
                falsifier="No trusted improvement over the best member alone.",
                prohibition_conditions=("evaluator_or_split_change_required",),
            ),
            MethodCard(
                method_id="model_lgbm_lambdarank_blend",
                family="model",
                summary="Blend a frozen LightGBM LambdaRank ranker into the FM parent as a per-user z-scored residual at alpha 0.70.",
                tags=("lightgbm", "lambdarank", "residual", "proven_recipe", "model"),
                cost_tier="medium",
                mechanism="final = FM + 0.70 * std_user(FM) * z_user(raw ranker); confirmed +0.0023 full-fidelity in a sibling evaluation of the same contract.",
                prerequisites=("baseline_parity", "objective_data_frame_verified"),
                allowed_data=(
                    "train_interactions",
                    "user_id",
                ...
                expected_effect="Roughly +0.002 primary over the FM parent with the frozen recipe.",
                falsifier="No trusted full-fidelity improvement with the predeclared frozen parameters.",
                prohibition_conditions=("evaluator_or_split_change_required",),
            ),
            MethodCard(
                method_id="model_gbdt_stack",
                family="model",
                status="known_negative",
                summary="Replace the served score with a LightGBM model that takes the FM parent score as an input feature.",
                tags=("lightgbm", "gbdt", "stacking", "replacement_capable", "model"),
                cost_tier="high",
                mechanism="Boosted trees over permitted fields plus the FM score strictly contain the parent ordering while adding higher-order and non-monotone interactions.",
                prerequisites=("baseline_parity", "objective_data_frame_verified"),
                allowed_data=(
                    "train_interactions",
                    "user_id",
                ...
                expected_effect="Improve within-user ordering beyond what a bounded residual on the FM parent can express.",
                falsifier="No trusted full-fidelity improvement over the FM parent, or near-perfect rank correlation with the parent.",
                prohibition_conditions=("evaluator_or_split_change_required",),
            ),
            MethodCard(

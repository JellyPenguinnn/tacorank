...
**Traversal:** adaptive research plans above atomic experiments, using
score-guided depth-first search to deepen the best trusted branch before
backtracking. Deepen a supported mechanism, run its confirmation or ablation,
and close a plan when evidence falsifies it or its small experiment ceiling is
exhausted. New live deployments use at most two simultaneous lanes. The first
follows the normal outcome policy; the optional scouting lane must use a method
never tried anywhere in the run. Repeating a prior method on a new parent is
legal only when the outcome policy explicitly requests refinement or deepening.
